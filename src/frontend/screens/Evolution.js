...
function Evolution(props) {
    ...
    function Button(props) {
        const {colors} = useTheme();
        return (<TouchableOpacity onPress={props.onPress} disabled={props.disabled} style={{
            height: 50,
            width: 175,
            marginTop: 20,
    ...
    return (
        <>
            <SafeAreaView style={[styles(colors).headContainer]}>
                ...
                <TouchableOpacity disabled={selected != null} style={{
                    position: "absolute",
                    top: height * 0.20,
                    left: width * 0.35,
                }} onPress={() => handleAnimation(0, height * 0.10, 0, height * 0.20, width * 0.35)}>
                    <Animated.Image
                        style={[{
                            resizeMode: 'contain',
                            height: 150,
                            ...
                            transform: [{scale: zoomValue}, {translateX: xValue}, {translateY: yValue}, {rotate: spin}],
                            resizeMode: 'contain'
                        } : selected != null && {opacity: petOpacity}]}
                        source={EvolutionMapping[evolutionNames[0]]}/>
                </TouchableOpacity>
                <TouchableOpacity disabled={selected != null}
                                  style={{position: "absolute", top: height * 0.5, left: width * 0.05}}
                                  onPress={() => handleAnimation(width * 0.15, height * -0.05, 1, height * 0.5, width * 0.05)}>
                    <Animated.Image
                        style={[{resizeMode: 'contain', height: 150, width: 150, tintColor: 'black'}, selected == 1 ? {
                            transform: [{scale: zoomValue}, {translateX: xValue}, {translateY: yValue}, {rotate: spin}],
                            resizeMode: 'contain'
                        } : selected != null && {opacity: petOpacity}]}
                ...
                <TouchableOpacity disabled={selected != null}
                                  style={{position: "absolute", top: height * 0.5, left: width * 0.60}}
                                  onPress={() => handleAnimation(width * -0.13, height * -0.05, 2, height * 0.5, width * 0.60)}>
                    <Animated.Image
                        style={[{resizeMode: 'contain', height: 150, width: 150, tintColor: 'black'}, selected == 2 ? {
                            transform: [{scale: zoomValue}, {translateX: xValue}, {translateY: yValue}, {rotate: spin}],
                            resizeMode: 'stretch',
                            zIndex: 1
                        } : selected != null && {opacity: petOpacity}]}
                ...
                <Animated.View style={{opacity: backgroundOpacity, top: '70%', alignItems: 'center'}}>
                    {!showColor ? <>
                            <Button text="EVOLVE" disabled={buttonDisabled} onPress={() => evolve()}></Button>
                            <Button text="CANCEL" disabled={buttonDisabled} onPress={() => resetSelect()}></Button>
                        </>
                        :
                        <Button text="CONTINUE" disabled={buttonDisabled} onPress={() => props.navigation.goBack(null)}></Button>
                    }
