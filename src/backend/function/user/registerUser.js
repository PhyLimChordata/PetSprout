...
const user_regist = async(req,res) => {
    try{
        ...
        if(user)
            return res.status(401).json("User email exists");
...
const user_activation = async(req,res)=>{
    try {
        ...
        if(!mail_vali) return res.status(200).json("msg:" + "No validation email was send before");
        ...
        if(code !== mail_vali.veri_code) res.status(200).json("msg:" + "incorrect validation code");
...
function sendUserEmail(cnd,code){
    try{
        ...
        if(regEmail.test(cnd)){
            ...
            var html = "<div>http://127.0.0.1:5000/api/v1.0.0/user/activation/" + code + "/" + cnd + "</div>";
	        console.log(html);
	        var data = {
