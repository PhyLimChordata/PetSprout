...
module.exports = async (req, res) => {
	try {
		...
		userHabit.expValue = userHabit.expValue + 10;
		let newSchedule = habitFromDB.schedule;

		if(habitFromDB.todo.toString() === habitFromDB.times) {
			habitFromDB.continuous = habitFromDB.continuous + 1;
			let current = new Date(date);
		    let current_date = current.getDate();
		    let current_day = current.getDay();
			let index = null;
			if (current_day === 6) index = 0;
			else index = current_day + 1;
		    let interval = 1;
		    ...
		    let nextSignInDate = current.setDate(current_date + interval);
